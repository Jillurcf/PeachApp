...
const drink = {value:selectedOptions, is_show}
console.log(drink)
  useEffect(() => {
    ...
    if (storedData) {
      ...
      if (parsedData?.drink) {
        setSelectedOptions(parsedData?.drink); // Ensure conversion to Date object
      }
    }
  }, []);
  ...
  const handleContinue = () => {
    ...
    const updatedDataList = [...dataList, {drink: drink}];

    // Save updated data
    MMKV.setString('dataList', JSON.stringify(updatedDataList));

    // Navigate to the next screen
    navigation?.navigate('smokeStatus');
  };
