...
const religion = {value:selectedOptions, is_show}
console.log(religion)
  useEffect(() => {
    ...
    if (storedData) {
      ...
      if (parsedData?.religion) {
        setSelectedOptions(parsedData?.religion); // Ensure conversion to Date object
      }
    }
  }, []);
  ...
  const handleContinue = () => {
    ...
    const updatedDataList = [...dataList, {religion: religion}];

    // Save updated data
    MMKV.setString('dataList', JSON.stringify(updatedDataList));

    // Navigate to the next screen
    navigation?.navigate('drinkingStatus');
  };
