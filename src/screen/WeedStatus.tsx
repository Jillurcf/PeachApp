...
const WeedStatus = ({navigation}: NavigProps<null>) => {
  ...
  const smoke_weed = {value: selectedOptions, is_show};
  console.log(smoke_weed)
  useEffect(() => {
    ...
    if (storedData) {
      ...
      if (parsedData?.smoke_weed) {
        setSelectedOptions(parsedData?.smoke_weed); // Ensure conversion to Date object
      }
    }
  }, []);
  ...
  const handleContinue = () => {
    ...
    const updatedDataList = [...dataList, {smoke_weed: smoke_weed}];

    // Save updated data
    MMKV.setString('dataList', JSON.stringify(updatedDataList));

    // Navigate to the next screen
    navigation?.navigate('drugStatus');
  };
